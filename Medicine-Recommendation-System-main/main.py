...
import pandas as pd
import pickle
from sample_gemini import generate,get_food_info
...
def get_predicted_value(patient_symptoms):
    ...
    if unknown_symptoms:
        return f"Unknown symptoms: {', '.join(unknown_symptoms)}. Please provide valid symptoms."
...
def home():
    ...
    if isinstance(predicted_disease, str) and "Unknown symptoms" in predicted_disease:
        return render_template('model.html', message=predicted_disease)
...
def calculate_bmi():
    ...
    height = float(data['height']) / 100  # Convert height to meters

    bmi = round(weight / (height ** 2), 2)
    suggestions = generate(f"Provide health and diet suggestions for a person with a BMI of {bmi}. "
                           f"Format the output as a list of concise points. No explanations or extra information, just actionable suggestions. "
                           f"Example: 'Follow a good diet that includes more fiber', 'Exercise daily'.")
    print(suggestions)
    return jsonify({'bmi': bmi, 'suggestions': suggestions})
@app.route('/kyf', methods=['GET'])
def know_your_food():
    return render_template('know_your_food.html')
@app.route('/kyf_details', methods=['GET'])
def kyf_details():
    ...
    try:
        details = get_food_info(food_item)
...
def get_exercise():
    return render_template('exercise_guidance.html')
